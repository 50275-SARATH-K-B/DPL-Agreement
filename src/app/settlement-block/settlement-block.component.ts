...
@Component({
  selector: 'app-settlement-block',
  templateUrl: './settlement-block.component.html',
  styleUrls: ['./settlement-block.component.scss']
})
export class SettlementBlockComponent implements OnInit {
  ...
  public confirm(settlementForm): void {
    ...
    this.settings.loadingSpinner = true;
    console.log(params)
    this.repaymentService.settlementblock(params).subscribe(res => {
      this.settings.loadingSpinner = false;
      if (!!res && res['status'].code == 1) {
        this.DisplayMessage('Block Requested Successfully', 'Success');
